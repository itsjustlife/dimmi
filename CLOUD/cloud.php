...
<body class="door-body">
  <header class="door-header">
    <div class="door-title">Door</div>
    <nav class="door-actions">
      <a class="door-link" href="cloud.php?mode=classic">Back to classic panels</a>
      <button id="door-refresh" type="button" class="door-button">Refresh</button>
    </nav>
  </header>
  <main class="door-main">
    <div class="door-toolbar">
      <nav id="door-breadcrumb" class="door-breadcrumb" aria-label="Breadcrumb"></nav>
      <label class="door-search" for="door-search-input">
        <span class="sr-only">Search this folder</span>
        <input id="door-search-input" type="search" placeholder="Filter items" autocomplete="off">
      </label>
    </div>
    <section id="door-grid" class="door-grid" role="grid" aria-label="Items"></section>
  </main>
  <aside id="door-drawer" class="door-drawer hidden" aria-label="Preview">
    <header class="door-drawer-header">
      <div>
        <h2 id="door-drawer-title" class="door-drawer-title">Preview</h2>
        <p id="door-drawer-path" class="door-drawer-path"></p>
      </div>
      <button id="door-drawer-close" type="button" class="door-button">Close</button>
    </header>
    <div class="door-drawer-tabs" role="tablist">
      <button id="door-tab-web" type="button" role="tab" aria-selected="true" class="door-tab door-tab-active">WEB</button>
      <button id="door-tab-raw" type="button" role="tab" aria-selected="false" class="door-tab">RAW</button>
      <button id="door-save" type="button" class="door-button door-save hidden">Save</button>
    </div>
    <section id="door-web-view" class="door-drawer-pane" role="tabpanel" aria-labelledby="door-tab-web"></section>
    <textarea id="door-raw-view" class="door-drawer-pane hidden" role="tabpanel" aria-labelledby="door-tab-raw"></textarea>
  </aside>
  <script>
    window.__csrf = {$csrf};
  </script>
  <script src="assets/js/preview-service.js"></script>
...
if ($mode === 'door' && isset($_GET['fs'])) {
  if (!$authed) bad('Unauthorized',401);
  if (!$ROOT || !is_dir($ROOT)) bad('Invalid ROOT (set near top of file).',500);
  $op = strtolower($_GET['fs']);
  if ($op === 'list') {
    $pathParam = $_GET['path'] ?? '';
    $safe = safe_abs($pathParam);
    if ($safe === false) bad('Invalid path');
    if (!is_dir($safe)) bad('Not a directory');
    $rel = rel_of($safe);
    $parent = null;
    if ($rel !== '') {
      $parts = explode('/', $rel);
    ...
    $folders = [];
    $files = [];
    foreach (scandir($safe) as $name) {
      if ($name === '.' || $name === '..') continue;
      $full = $safe.'/'.$name;
      $relItem = rel_of($full);
      if (is_dir($full)) {
        $folders[] = ['name'=>$name,'path'=>$relItem];
      } else {
        $files[] = ['name'=>$name,'path'=>$relItem,'ext'=>strtolower(pathinfo($name, PATHINFO_EXTENSION))];
      }
    }
    j(['ok'=>true,'cwd'=>$rel,'parent'=>$parent,'folders'=>$folders,'files'=>$files]);
  }
  bad('Unknown fs action',404);
}

$mode = isset($_GET['mode']) ? strtolower($_GET['mode']) : 'classic';
if ($mode === 'door') {
...
<body class="h-screen flex flex-col bg-gray-50 text-gray-800 overflow-x-hidden">
  <header class="flex items-center gap-4 p-4 bg-white shadow">
    ...
    <a href="cloud.php?mode=door" class="px-3 py-1 text-sm border rounded border-gray-300 text-gray-700 hover:bg-gray-100">Open Door</a>
...
async function openFile(rel,name,size,mtime){
  ...
  if(isImg){
    ...
    if(window.ClassicPreview) ClassicPreview.setFile('', {});
    hideTree();
    return;
  }else{
    if (imgWrap) imgWrap.classList.add('hidden');
  ...
  if(isStruct){
    ...
    try{
      ...
      if(p.ok){
        ...
        if(window.ClassicPreview) ClassicPreview.setFile(currentFile, { nodes: [], docTitle: '', helpers:{} });
    ...
    if(contentTabs) contentTabs.classList.add('hidden');
    if(contentPreview) contentPreview.classList.add('hidden');
    if(window.ClassicPreview) ClassicPreview.setFile(rel, {});
  }
}
