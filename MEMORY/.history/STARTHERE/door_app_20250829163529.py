...
from DimmiD.dimmi_core import DimmiRunner  # Add this import
...
class DoorApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        tk.Button(toolbar, text="ASK AI", command=self.ask_ai).pack(side=tk.LEFT)  # <--- NEW BUTTON
    ...
    def ask_ai(self):
        def do_ask():
            ...
            ai_output.insert("1.0", reply)
            ai_output.config(state="disabled")
            ai_btn.config(state="normal")

        win = tk.Toplevel(self)
        win.title("Ask Dimmi AI")
        tk.Label(win, text="Prompt:").pack(anchor="w")
        ai_entry = tk.Entry(win, width=60)
        ai_entry.pack(fill=tk.X, padx=8, pady=4)
        ai_btn = tk.Button(win, text="Ask", command=do_ask)
        ai_btn.pack(pady=4)
        tk.Label(win, text="AI Reply:").pack(anchor="w")
        ai_output = tk.Text(win, height=8, width=60, state="disabled")
        ai_output.pack(fill=tk.BOTH, padx=8, pady=4)
